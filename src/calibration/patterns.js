...
const getPattern = (numTargets, patterns) => {
...
const getPatternCoordsInPct = ({
  ...
  switch (type) {
    ...
    case 'validation':
      ...
      break;
    default:
      throw new error('No patterns for this task given,' +
        'task needs to be \'calibration\' or \'validation\''
      );
  }
};

/*
 * For debugging target patterns.
 */
const showGazePattern = (numTargets, root, validation=false) => {
  const container = getCalibrationContainer(root);
  let gazeTargets;
  if (validation) {
    gazeTargets = gatValidationPatternCoordsInPercent(numTargets);
  } else {
    gazeTargets = getCalibrationPatternCoordsInPercent(numTargets);
  }
  gazeTargets.forEach((gazeTargetCoords) => {
    drawGazeTarget(container, gazeTargetCoords);
  });
};

export {getPatternCoordsInPct, showGazePattern};
